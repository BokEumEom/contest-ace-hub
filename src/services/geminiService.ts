...
`;
  ...
  async extractContestInfoFromText(textContent: string): Promise<{
    title: string;
    organization: string;
    deadline: string;
    category: string;
    ...
    const prompt = `
다음은 공모전 정보가 포함된 텍스트입니다. 이 텍스트에서 공모전 정보를 정확히 추출해주세요.

특히 출품 형식(submissionFormat)은 파일 형식, 크기, 해상도, 용량 제한, 페이지 수 등을 구체적으로 포함해야 합니다.

텍스트 내용:
${textContent.substring(0, 3000)} // 내용이 너무 길면 앞부분만 사용
